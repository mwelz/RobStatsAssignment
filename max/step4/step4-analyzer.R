...
load(paste0(getwd(), "/max/step4/step4-simdata.Rdata"))
...
for(d in d.set){
  ...
  pdf(file = paste0(getwd(),
                    "/max/step4/plot-fixed-d/plot_d=", d, ".pdf"))
  plot(tests.ave[idx, "n"], tests.ave[idx, "classic_pval"], 
       xlab = "sample size n", ylab = "power", type = "l", 
...
for(n in n.set){
  ...
  pdf(file = paste0(getwd(),
                    "/max/step4/plot-fixed-n/plot_n=", n, ".pdf"))
  plot(tests.ave[idx, "d"], tests.ave[idx, "classic_pval"], 
       xlab = "dimension d", ylab = "power", type = "l", 
